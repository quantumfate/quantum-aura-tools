...
--
-- This replaces HyperTools' single ad-hoc nil-filling function: adding a field
-- means appending one numbered step, never editing a growing monolith.
--
...
function QAT.RunMigrations(sv)
    ...
    for v = from, to - 1 do
        ...
        if step then
            step(sv)
            if QAT.Log then QAT.Log("migrated schema %d -> %d", v, v + 1) end
        end
    end
